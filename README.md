...
| **College Football hub** (`#/cfb`) | The NFL shell on the FBS feed: every game with win-probability, the AP Top 25 with the twelve-team playoff line, 138 teams by conference, rosters, career pages, and the Aussies (mostly punters) |
| **Cricket hub** (`#/cricket`) | Matches merged across the competitions in `data/cricket.json` (ESPN cricket ids = Cricinfo series ids), the BBL ladder, Cricinfo/Guardian/SMH news. Add each Australian series as it's announced |
| **Tour hubs** (`#/tennis`, `#/f1`, `#/golf`, `#/ufc`) | One event-shaped renderer (`api/globalsports.py`): the live/next event with a countdown, its matches / sessions / leaderboard / fight card in Sydney time, F1's two championships, and every Australian in the field |
| **LA 2028** (`#/la2028`) | Countdown to the Opening Ceremony, key dates, the sports and where Australia plays, the Australians to watch — from `data/la2028.json` |
| **Aussies Abroad** (`/aussies`) | Every Australian across NFL, NBA, MLB, college football, tennis, golf, F1 and UFC on one page — headshots, hooks, and each one's next (or live) game in your time. Built from the roster scans and tour fields; `/api/aussies-abroad` (server fan-out, 7 s budget, `?league=` for a single code) |
...
**Hub loads.** `/api/hub?league=` bundles schedule, news, videos, ladder, leaders, form, finals and Aussies into one server-side fan-out (each optional module fails to null on its own, 6 s budget each); `loadHub` uses it and falls back to per-module calls. Saved games (`#/saved`) live in localStorage only — the copy says so.
...
| `data/cricket.json` | Cricket competitions to poll (Cricinfo series ids) + season copy |
| `data/la2028.json` | LA 2028 editorial: dates, facts, milestones, sports, Australians to watch |
